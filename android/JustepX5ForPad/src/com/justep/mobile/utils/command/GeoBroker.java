...
public class GeoBroker extends PluginAbstractImpl {
	...
	public CommandCallback execute(String action, JSONArray args,
		...
		try {
			if (action.equals("getCurrentLocation")) {
				this.getCurrentLocation(args.getBoolean(0), args.getInt(1),
						args.getInt(2));
	...
	public void onDestroy() {
		java.util.Set<Entry<String, GeoListener>> s = this.geoListeners
				.entrySet();
	...
	/**
	 * Get current location. The result is returned to JavaScript via a
	 * callback.
	 * 
	...
	public void getCurrentLocation(boolean enableHighAccuracy, int timeout,
		...
		// Create a geolocation listener just for getCurrentLocation and call it
		// "global"
		if (this.global == null) {
	 ...
	 * @return
	 */
	public String start(String key, boolean enableHighAccuracy, int timeout,
			int maximumAge) {
