 ...
 * 
 */
public class Capture extends PluginAbstractImpl {
		...
		if (action.equals("getFormatData")) {
			try {
				JSONObject obj = getFormatData(args.getString(0),
						args.getString(1));
				return new CommandCallback(CommandCallback.Status.OK, obj);
	...
	private void captureImage() {
		...
		File photo = new File(DirectoryManager.getTempDirectoryPath(ctx),
				"Capture.jpg");
		...
		if (resultCode == Activity.RESULT_OK) {
			...
			if (requestCode == CAPTURE_AUDIO) {
				...
				if (results.length() >= limit) {
					...
					this.success(
							new CommandCallback(CommandCallback.Status.OK,
									results,
									"justepApp.device.capture._castMediaFile"),
							this.callbackId);
				} else {
				...
				try {
					...
					if (results.length() >= limit) {
						...
						this.success(new CommandCallback(
								CommandCallback.Status.OK, results,
								"justepApp.device.capture._castMediaFile"),
								this.callbackId);
					} else {
		...
		// If canceled
			...
			if (results.length() > 0) {
				this.success(new CommandCallback(CommandCallback.Status.OK,
						results, "justepApp.device.capture._castMediaFile"),
						this.callbackId);
			}
