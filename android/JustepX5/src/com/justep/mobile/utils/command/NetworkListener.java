...
public class NetworkListener implements LocationListener {
	 ...
	 * @param m
	 */
	public NetworkListener(PortalActivity ctx, int interval, GeoListener m) {
		this.owner = m;
		this.mCtx = ctx;
	...
	public void onStatusChanged(String provider, int status, Bundle extras) {
		...
		if (status == 0) {
			...
			System.out
					.println("NetworkListener: " + provider + " is Available");
		}
	}
	...
	public void onLocationChanged(Location location) {
		...
		// The GPS is the primary form of Geolocation in JustepApp.
	...
	public void start(int interval) {
		if (!this.running) {
			this.running = true;
			this.mLocMan.requestLocationUpdates(
					LocationManager.NETWORK_PROVIDER, interval, 0, this);
			this.getLocation();

			// If Network provider has data but GPS provider doesn't, then send
			// ours
