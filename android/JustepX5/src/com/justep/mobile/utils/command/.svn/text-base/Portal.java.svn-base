...
public class Portal extends PluginAbstractImpl {
	public CommandCallback execute(String action, JSONArray args,
		...
		String result = "";
		try {
			if (action.equals("hideToolbar")) {
				this.setToolbarVisible(false);
			} else if (action.equals("showToolbar")) {
				this.setToolbarVisible(true);
			} else if (action.equals("swicthPage")) {
				((PortalActivity) this.ctx).switchPage(args.getString(0),
						args.getString(1));
			} else if (action.equals("setToolbarHeight")) {
				this.setToolbarHeight(args.getInt(0));
	...
	public void setToolbarVisible(final boolean flag) {
		ctx.findViewById(R.id.webView2).setVisibility(flag ? 0 : 8);
		ImageView btn = ((ImageView) ctx.findViewById(R.id.button1));
		btn.setVisibility(flag ? 8 : 0);
	}

	public void setToolbarHeight(final int height) {
		WebView view2 = (WebView) ctx.findViewById(R.id.webView2);
		RelativeLayout.LayoutParams linearParams = (LayoutParams) view2
